...
class PresetDates:
    year_start: datetime.datetime = MarketDaysHelper.get_this_or_next_market_day(
        datetime.date(year=test_year, month=1, day=1)
    )
    year_end: datetime.datetime = MarketDaysHelper.get_this_or_previous_market_day(
        datetime.date(year=test_year, month=12, day=31)
    )
    dec_start: datetime.datetime = MarketDaysHelper.get_this_or_next_market_day(
        datetime.date(year=test_year, month=12, day=1)
    )
    dec_end: datetime.datetime = year_end
    q4_start: datetime.datetime = MarketDaysHelper.get_this_or_next_market_day(
        datetime.date(year=test_year, month=10, day=1)
    )
    q4_end: datetime.datetime = year_end
    recent_day: datetime.datetime = MarketDaysHelper.get_this_or_previous_market_day(
...
def check_cols_present(data: pd.DataFrame, cols_to_test: [int], case_identifier: str):
    for col_name in cols_to_test:
        if not col_name in data.columns:
            assert pytest.fail(
                f"column {col_name} not present in {case_identifier} data"
            )
...
def check_price_cols_present(data: pd.DataFrame, period: str):
    check_cols_present(
        data,
        [
            col_template.substitute({"period": period})
            for col_template in TypeHelper.get_class_static_values(BasePriceColumns)
        ],
        period,
    )
...
def check_col_values(data: pd.DataFrame, col_value_pairs: dict):
    for col_name in col_value_pairs:
        if isinstance(col_value_pairs[col_name], float):
            if not float(data[col_name].values[0]) == pytest.approx(
                col_value_pairs[col_name]
            ):
                assert pytest.fail(
                    f"column `{col_name}` value check: {data[col_name].values[0]} <> {col_value_pairs[col_name]}"
                )
        else:
