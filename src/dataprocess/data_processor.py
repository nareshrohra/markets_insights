...
from calculations.base import CalculationWorker, CalculationPipeline, ValueCrosseAboveFlagWorker, ValueCrossedBelowFlagWorker
...
class CalculationPipelineBuilder:
  def create_sma_calculation_pipeline(windows = [50, 100, 200]):
    ...
    for window in windows:
      pipeline.add_calculation_worker(SMACalculationWorker(window))
    return pipeline
  
  def create_rsi_calculation_pipeline(crossing_above_flag_value = None, crossing_below_flag_value = None):
    pipeline = CalculationPipeline()
    pipeline.add_calculation_worker(RsiCalculationWorker(14))
    if crossing_above_flag_value is not None:
      pipeline.add_calculation_worker(ValueCrosseAboveFlagWorker(CalculatedColumns.RelativeStrengthIndex, crossing_above_flag_value))
...
class SMACalculationWorker (CalculationWorker):
  ...
  def add_calculated_columns(self, data):
    ...
    data[self._column_name] = identifier_grouped_data['Close'].transform(
...
class HistoricalDataProcessor(DataProcessor):
  ...
  def run_calculation_pipelines(self):
    self.calculation_pipelines.run(self.dataset.get_daily_data())
  ...
  def process(self, reader: DataReader, options: dict):
    ...
    #historical_data['Identifier'] = historical_data['Identifier'].str.upper()
